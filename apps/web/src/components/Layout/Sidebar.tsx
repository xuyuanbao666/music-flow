      ...
      <nav className="flex-1 px-3">
        ...
        <div className="mt-8">
          <div className="flex items-center justify-between px-4 mb-3">
            <h2 className="text-gray-600 dark:text-gray-400 text-sm font-semibold uppercase tracking-wider">我的歌单</h2>
            <Button variant="ghost" size="sm" onClick={handleCreatePlaylist} className="text-gray-600 dark:text-gray-400 hover:text-gray-900 dark:hover:text-white">+</Button>
          </div>
          <div className="space-y-1">
            {playlists.map(playlist => <button key={playlist.id} onClick={() => onNavigate(`playlist/${playlist.id}`)} className={`w-full flex items-center gap-3 px-4 py-2 rounded-lg transition-colors ${currentPage === `playlist/${playlist.id}` ? 'bg-gray-200 dark:bg-gray-800 text-gray-900 dark:text-white' : 'text-gray-600 dark:text-gray-400 hover:text-gray-900 dark:hover:text-white hover:bg-gray-200 dark:hover:bg-gray-800'}`}><span className="text-lg">📋</span><span className="truncate">{playlist.name}</span></button>)}
          </div>
        </div>
      </nav>
    </div>
